...
import com.app.schoolapp.adapter.AdapterEventCalendar;
...
public class Eventcalendar extends AppCompatActivity implements OnCustomItemClicListener, ApiResponse {

    private Context context;
    private Toolbar toolbar;
    private RecyclerView recycler_list;
    private AdapterEventCalendar adapterEventCalendar;
    private ArrayList<ModelData> arrayList;
    private SwipeRefreshLayout swipeRefreshLayout;
    private BroadcastReceiver broadcastReceiver;
    private List<EventDataModal> eventDataModal;
...
/*        modelDoctor = new ModelData();
        ...
        adapterEventCalendar = new AdapterEventCalendar(context, this, arrayList);
        recycler_list.setAdapter(adapterEventCalendar);*/
        getEventData();
    }
    ...
    private void getEventData() {
        try {
            if (AppUtils.isNetworkAvailable(context)) {
                String url = getResources().getString(R.string.base_url) + getResources().getString(R.string.event_list) + "?client_id=" + AppUtils.getUserId(context);
    ...
    private void setEventData() {
        adapterEventCalendar = new AdapterEventCalendar(context, this, eventDataModal);
        recycler_list.setAdapter(adapterEventCalendar);
    }
            ...
            if (success == 1 && error == 0) {
                ...
                if (length > 0) {
                    for (int i = 0; i < length; i++) {
                        ...
                        String eve = jsonObject.getString("eve");
                        String writtenBy = jsonObject.getString("writtenBy");
                        eventDataModal.add(new EventDataModal(id, title, content, eve, created, writtenBy));
                    }
                    setEventData();
                }
            } else {
